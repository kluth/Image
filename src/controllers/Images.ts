...
const imageExists = (
	...
	const filePath = width
		? `${prefix}_${width}x${height}${image}`
		: `${prefix}_${image}`
...
const ImagesController = {
	...
	resize: (req: Request, res: Response) => {
		...
		imageExists(id, 'thumb', +width, +height)
			? res.sendFile(`./images/thumb_${width}x${height}${id}`)
			: sharp(`./images/${id}`)
	...
	randomResizeAll: (req: Request, res: Response) => {
		...
		// and send the result to the client
		const errors: Array<string> = []
		const infos: Array<sharp.OutputInfo> = []
		fs.readdir(`./images`, (err, files) => {
			if (err) {
				errors.push(err.message)
			}
			files.forEach((file) => {
				...
				imageExists(file, 'thumb', width, height)
					? res.sendFile(`./images/thumb_${width}x${height}${file}`)
					: sharp(`./images/${file}`)
							.resize(+width, +height)
							.toFile(
								`./images/thumb_${width}x${height}${file}`,
								(err, info) => {
									if (err) {
										errors.push(err.message)
									}
	...
	monochromeAll: (req: Request, res: Response) => {
		...
		fs.readdir(`./images`, (err, files) => {
			...
			files.forEach((file) => {
				imageExists(file, 'monochrome')
					? res.sendFile(`./images/monochrome_${file}`)
					: sharp(`./images/${file}`)
							.grayscale()
	...
	monochrome: (req: Request, res: Response) => {
		...
		// and send the result to the client
		imageExists(id, 'monochrome')
			? res.sendFile(`./images/monochrome_${id}`)
			: sharp(`./images/${id}`)
					.grayscale()
